...
export default function EZPassPage(): React.ReactElement {
  const totalTolls = MOCK_TOLLS.reduce((sum, t) => sum + t.amount, 0);
  const verifiedCount = MOCK_TOLLS.filter(t => t.status === "verified").length;
  const pendingCount = MOCK_TOLLS.filter(t => t.status !== "verified").length;
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="flex-1 space-y-4 px-4 pt-4">
        ...
        <div className="space-y-2">
          <h3 className="stat-label">Peajes Detectados</h3>
          {MOCK_TOLLS.map((toll) => (
